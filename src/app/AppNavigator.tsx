...
export function AppNavigator() {
  ...
  const open: OpenContent = useCallback((item, context) => {
    ...
    setDetailStack((stack) =>
      stack.some((entry) => entry.id === item.id && entry.type === item.type)
  ...
  const goToEmergencies = useCallback(() => {
    setDetailStack([]);
    setDetailContext(null);
    setTab('more');
  }, []);
  ...
  const openFallback = useCallback((destination: DeepLinkDestination) => {
    void openExternalUrl(fallbackWebUrl(destination));
  }, []);
  const handleDestination = useCallback(
    (destination: DeepLinkDestination) => {
      switch (destination.kind) {
        case 'content':
          getContentDetail(destination.id)
            .then((item) => {
              setDetailContext(null);
              setDetailStack([item]);
            })
            .catch(() => openFallback(destination));
          return;
        case 'directory':
          getDirectoryEntry(destination.id)
            .then((item) => {
              setDetailContext(null);
              setDetailStack([item]);
            })
        ...
        case 'permalink':
          resolveContentBySlug(destination.slug)
            .then((item) => (item ? (setDetailContext(null), setDetailStack([item])) : openFallback(destination)))
            .catch(() => openFallback(destination));
          return;
        case 'home':
          setDetailStack([]);
        ...
        case 'unknown':
        default:
          openFallback(destination);
      }
    },
    [goToExplore, openFallback],
  );
  useDeepLinkRouter(handleDestination);
  if (detail)
    return (
      <SlideInScreen key={detailStack.length} onDismiss={closeDetail}>
        {(dismiss) => (
          <ContentDetailScreen
            item={detail}
            onBack={dismiss}
  ...
  const screen =
    tab === 'home' ? (
      <HomeScreen onOpen={open} onSearch={() => setTab('search')} onExplore={(collection) => goToExplore({ collection: collection ?? null })} onEmergencies={goToEmergencies} />
    ) : tab === 'explore' ? (
      <ExploreScreen
...
function makeStyles() {
  return StyleSheet.create({ root: { flex: 1 } });
}
