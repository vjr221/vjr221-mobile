...
export function AppNavigator() {
  ...
  const screen =
    ...
    tab === 'explore' ? <ExploreScreen key={exploreSeedVersion} onOpen={open} initialCollection={exploreSeed.collection ?? undefined} initialGeoView={exploreSeed.geoView} initialDirectoryCategory={exploreSeed.directoryCategory} /> :
    tab === 'search' ? <SearchScreen onOpen={open} /> :
    tab === 'favorites' ? <FavoritesScreen onOpen={open} /> : <MoreScreen locale={locale} onLocale={setLocale} />;
    ...
    <View accessibilityRole="tablist" style={styles.tabBarWrap}>
      <View style={styles.tabs}>
        {TABS.map((key) => {
          ...
          return <Pressable accessibilityRole="tab" accessibilityState={{ selected: active }} accessibilityLabel={t(key)} key={key} onPress={() => onChange(key)} style={styles.tab}>
            <View style={[styles.tabIconWrap, active && styles.tabIconWrapActive]}><Icon name={key === 'favorites' && items.length > 0 && active ? 'heartFilled' : TAB_ICON[key]} size={20} color={active ? colors.terreStrong : colors.inkSoft} /></View>
