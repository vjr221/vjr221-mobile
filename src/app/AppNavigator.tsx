...
export function AppNavigator() {
  ...
  const openFallback = useCallback((destination: DeepLinkDestination) => { void openExternalUrl(fallbackWebUrl(destination)); }, []);
  const handleDestination = useCallback((destination: DeepLinkDestination) => { switch (destination.kind) { case 'content': getContentDetail(destination.id).then((item) => { setDetailContext(null); setDetailStack([item]); }).catch(() => openFallback(destination)); return; case 'directory': getDirectoryEntry(destination.id).then((item) => { setDetailContext(null); setDetailStack([item]); }).catch(() => openFallback(destination)); return; case 'directory-category': goToExplore({ collection: 'directory', directoryCategory: destination.slug || undefined }); return; case 'region': goToExplore({ collection: 'regions', geoView: { kind: 'region', id: destination.id } }); return; case 'department': goToExplore({ collection: 'regions', geoView: { kind: 'department', id: destination.id } }); return; case 'commune': goToExplore({ collection: 'regions', geoView: { kind: 'commune', id: destination.id } }); return; case 'village': goToExplore({ collection: 'regions', geoView: { kind: 'village', id: destination.id } }); return; case 'category': goToExplore({ collection: null }); return; case 'permalink': resolveContentBySlug(destination.slug).then((item) => item ? (setDetailContext(null), setDetailStack([item])) : openFallback(destination)).catch(() => openFallback(destination)); return; case 'home': setDetailStack([]); setDetailContext(null); setTab('home'); return; case 'unknown': default: openFallback(destination); } }, [goToExplore, openFallback]);
