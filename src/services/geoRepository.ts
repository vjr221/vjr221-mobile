...
interface RawGeoItem {
  ...
  title: string;
  excerpt: string | null;
  /** Wolof — présents dès que renseignés côté WordPress (vague 8). */
...
function toWolofFields(raw: RawGeoItem): { titleWo?: string | null; excerptWo?: string | null } {
  const titleWo = raw.title_wo ? decodeHtmlEntities(String(raw.title_wo), { trim: true }) : null;
  const excerptWo = raw.excerpt_wo ? cleanExcerpt(String(raw.excerpt_wo)) : null;
  return {
    titleWo: titleWo || null,
    excerptWo: excerptWo || null,
  };
}

export function toRegion(raw: RawGeoItem): Region {
  const wo = toWolofFields(raw);
  return {
    kind: 'region',
    id: raw.id,
    slug: raw.slug,
    ...
    titleWo: wo.titleWo,
    excerptWo: wo.excerptWo,
    permalink: raw.permalink,
    image: normalizeRemoteImage(raw.image),
    gps: raw.gps,
    infos: toInfos(raw.infos),
  };
}
...
/** Répare content_wo aplati côté CMS (ex. "### JëmmalnnTexte" → sauts de ligne réels). */
function normalizeWolofBody(value: string | null | undefined): string | null {
  if (!value) return null;
...
function toCta(raw: RawGeoDetail['cta']): GeoDetail<GeoEntity>['cta'] {
  ...
  return {
    ...
    buttonLabel: decodeHtmlEntities(raw.button_label),
...
function toDetail<T extends GeoEntity>(raw: RawGeoDetail, mapper: (r: RawGeoItem) => T): GeoDetail<T> {
  ...
  const contentWo = raw.content_wo
    ? normalizeWolofBody(decodeHtmlEntities(String(raw.content_wo), { trim: false }))
    : null;
  return {
    entity: mapper(raw),
    ...
    contentWo,
    gallery: (raw.galerie ?? [])
      .map((image) => normalizeRemoteImage(image))
      .filter((image): image is RemoteImage => Boolean(image)),
    usefulLinks: (raw.liens_utiles ?? []) as UsefulLink[],
    cta: toCta(raw.cta),
  };
}
