...
export const decodeHtmlEntities = (value: string, options: { trim?: boolean } = {}): string => {
  ...
  if (/[ÃÂâð][\x80-\xBF]/.test(result)) {
    try {
      const bytes = Array.from(result).map((char) => (char.charCodeAt(0) <= 255 ? `%${char.charCodeAt(0).toString(16).padStart(2, '0')}` : encodeURIComponent(char))).join('');
      const recovered = decodeURIComponent(bytes);
      if (recovered && !/[ÃÂâð][\x80-\xBF]/.test(recovered)) result = recovered;
    } catch { /* ignore */ }
  }
...
export const stripHtml = (html: string): string => {
  const decoded = decodeHtmlEntities(html, { trim: false });
...
export const toContentItem = (post: WordPressPost): ContentItem => {
  ...
  return {
    ...
    contentWo: base.contentWo?.trim() || local?.contentWo,
  };
};
...
export function getHomeFeed(): Promise<CollectionResult> {
  return getPosts('per_page=6&_embed', 'home:feed');
}

export function searchContent(term: string, page = 1): Promise<CollectionResult> {
  const q = term.trim();
  if (!q) return Promise.resolve({ items: [], fromCache: false, stale: false });
  return getPosts(`per_page=12&page=${page}&search=${encodeURIComponent(q)}&_embed`, `search:${q}:${page}`);
}
...
function toLieuContentItem(raw: RawLieuContentItem): ContentItem {
  return {
    ...
    excerpt: raw.excerpt ? stripHtml(raw.excerpt) : undefined,
    type: (raw.type as ContentType) || 'news',
    url: raw.permalink,
...
export async function getLieuContents(lieuId: number): Promise<CollectionResult> {
  const result = await withCacheFallback(`lieu:${lieuId}:contenus`, CACHE_TTL, async () => {
