    ...
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <View style={styles.grid}>
        {categories.map((category) => (
          <View key={category.id} style={[styles.tile, !category.available && styles.unavailable]}>
            <Text style={styles.icon}>{category.emoji}</Text>
            <Text style={styles.name}>{t(category.labelKey as TranslationKey)}</Text>
            <Text style={styles.status}>{category.available ? t('available') : t('comingSoon')}</Text>
          </View>
        ))}
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  content: { padding: spacing.md, paddingBottom: 105, backgroundColor: colors.surface, flexGrow: 1 },
