...
const TYPE_LABEL_KEY: Partial<Record<ContentItem['type'], TranslationKey>> = {
  news: 'news', tourism: 'tourism', heritage: 'heritage', gastronomy: 'gastronomy', people: 'people',
  directory: 'directory', history: 'history', nature: 'nature', culture: 'culture', events: 'events',
};
  ...
  return (
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={styles.heroWrap}>
        {item.imageUrl ? <Image accessibilityIgnoresInvertColors source={{ uri: item.imageUrl }} style={styles.hero} contentFit="cover" cachePolicy="memory-disk" transition={220} /> : <View style={[styles.hero, styles.heroFallback]}><Icon name="image" size={38} color={colors.line} /></View>}
        <View style={styles.heroShade} pointerEvents="none" />
        <Pressable onPress={onBack} accessibilityRole="button" accessibilityLabel={t('back')} hitSlop={8} style={styles.heroBack}><Icon name="chevronLeft" size={22} color={colors.ink} /></Pressable>
      </View>
      <View style={styles.body}>
        ...
        <View style={styles.actions}><Button variant="primary" onPress={toggleSaved} style={styles.primaryAction}>{saved ? t('saved') : t('save')}</Button><Button variant="secondary" onPress={share}>{t('share')}</Button></View>
        {practicalRows.length ? <View style={styles.practicalCard}>{practicalRows.map((row, index) => <Pressable key={`${row.label}-${row.value}`} accessibilityRole={row.onPress ? 'button' : undefined} accessibilityLabel={`${row.label}: ${row.value}`} onPress={row.onPress} disabled={!row.onPress} style={[styles.practicalRow, index === practicalRows.length - 1 && styles.practicalRowLast]}><Icon name={row.icon} size={16} color={colors.inkSoft} /><Text style={styles.practicalLabel}>{row.label}</Text><Text selectable numberOfLines={2} style={styles.practicalValue}>{row.value}</Text></Pressable>)}</View> : null}
        <LocationPreview coordinates={coordinates} label={item.title} />
        {practical?.website ? <Pressable accessibilityRole="link" accessibilityLabel={t('website')} style={styles.link} onPress={() => { void openExternalUrl(practical.website!); }}><Text style={styles.linkText}>{t('website')}</Text><Icon name="chevronRight" size={13} color={colors.terreStrong} /></Pressable> : item.url ? <Pressable accessibilityRole="link" accessibilityLabel={t('website')} style={styles.link} onPress={() => { void openExternalUrl(item.url!); }}><Text style={styles.linkText}>{t('website')}</Text><Icon name="chevronRight" size={13} color={colors.terreStrong} /></Pressable> : null}
        {related.length ? <View style={styles.relatedSection}><View style={styles.sectionHeadingRow}><Text style={styles.sectionTitle}>{t('related')}</Text><Text style={styles.sectionHint}>{related.length}</Text></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.relatedList}>{related.map((candidate) => <Pressable key={`${candidate.type}-${candidate.id}`} accessibilityRole="button" accessibilityLabel={candidate.title} onPress={() => onOpen?.(candidate)} style={({ pressed }) => [styles.relatedCard, pressed && styles.relatedPressed]}>{candidate.thumbnailUrl || candidate.imageUrl ? <Image source={{ uri: candidate.thumbnailUrl ?? candidate.imageUrl }} style={styles.relatedImage} contentFit="cover" cachePolicy="memory-disk" transition={160} /> : <View style={[styles.relatedImage, styles.relatedFallback]}><Icon name="image" size={20} color={colors.line} /></View>}<Text numberOfLines={2} style={styles.relatedTitle}>{candidate.title}</Text><Text style={styles.relatedType}>{candidate.type.toUpperCase()}</Text></Pressable>)}</ScrollView></View> : null}
      </View>
    </ScrollView>
  );
}
...
function makeStyles(colors: ReturnType<typeof useTheme>['colors']) {
  return StyleSheet.create({
    ...
    ctaBox: { backgroundColor: colors.surfaceSoft, borderRadius: radii.lg, marginTop: spacing.lg, padding: spacing.md, gap: 6 },
