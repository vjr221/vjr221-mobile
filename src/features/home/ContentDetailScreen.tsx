...
export function ContentDetailScreen({ item, onBack, onOpen, navigationContext, onNavigate }: Props) {
  ...
  const [relatedState, setRelatedState] = useState<{ key: string; items: ContentItem[] }>({
  ...
  useEffect(() => {
    ...
    getRelatedContent(item)
      .then((items) => {
        if (active) setRelatedState({ key: `${item.type}-${item.id}`, items });
      })
      .catch(() => {
        if (active) setRelatedState({ key: `${item.type}-${item.id}`, items: [] });
      });
  ...
  const whatsapp = () => {
    if (!practical?.whatsapp) return;
    const url = buildWhatsAppUrl(practical.whatsapp) ?? practical.whatsapp;
    void openExternalUrl(url, 'whatsapp');
  };
  const toggleSaved = () => {
    void import('expo-haptics').then((H) => H.impactAsync(saved ? H.ImpactFeedbackStyle.Light : H.ImpactFeedbackStyle.Medium)).catch(() => {});
    toggle(item);
  };
  ...
  const practicalRows: {
    icon: 'phone' | 'pin' | 'clock' | 'mail';
    label: string;
    value: string;
    onPress?: () => void;
  }[] = [];
  if (practical?.phone) practicalRows.push({ icon: 'phone', label: t('call'), value: practical.phone, onPress: call });
  if (practical?.whatsapp)
    practicalRows.push({ icon: 'phone', label: t('whatsapp'), value: practical.whatsapp, onPress: whatsapp });
