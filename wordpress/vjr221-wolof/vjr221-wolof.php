...
define( 'VJR221_WO_VERSION', '1.0.1' );
...
function vjr221_wo_normalize_newlines( $value ) {
...
function vjr221_wo_sanitize_meta( $value ) {
	...
	$value = vjr221_wo_normalize_newlines( $value );
...
function vjr221_wo_rest_prepare_post( $response, $post, $request ) {
	...
	foreach ( VJR221_WO_META_KEYS as $key ) {
		...
		if ( is_string( $raw ) && $raw !== '' ) {
			if ( $key === 'content_wo' ) {
				$raw = vjr221_wo_normalize_newlines( $raw );
			}
			$data[ $key ] = $raw;
		} else {
			$data[ $key ] = null;
...
function vjr221_wo_filter_geo_item( $item, $post_id, $context = 'list' ) {
	...
	if ( $context === 'detail' ) {
		...
		if ( is_string( $content_wo ) && $content_wo !== '' ) {
			$item['content_wo'] = vjr221_wo_normalize_newlines( $content_wo );
		} else {
			$item['content_wo'] = null;
		}
	}
	return $item;
}
...
function vjr221_wo_enrich_vjr221_response( $result, $server, $request ) {
	...
	if ( ! empty( $data['id'] ) && isset( $data['slug'] ) ) {
		$result->set_data( vjr221_wo_filter_geo_item( $data, (int) $data['id'], 'detail' ) );
	}
	return $result;
}
...
function vjr221_wo_render_meta_box( $post ) {
	...
	if ( is_string( $content ) ) {
		$content = vjr221_wo_normalize_newlines( $content );
	}
	?>
	<p>
	...
	<p>
		...
		<span class="description"><?php esc_html_e( 'Markdown simple accepté (### Jëmmal).', 'vjr221-wolof' ); ?></span>
	</p>
	<?php
}
...
function vjr221_wo_render_seed_page() {
	...
	if ( isset( $_POST['vjr221_wo_seed_repair'] ) && check_admin_referer( 'vjr221_wo_repair' ) ) {
		$repaired = vjr221_wo_repair_content_newlines();
		$notice   = sprintf(
			__( 'Réparation terminée : %d content_wo normalisé(s).', 'vjr221-wolof' ),
			$repaired
		);
	}
	?>
	<div class="wrap">
		...
		<?php endif; ?>
		<p><?php esc_html_e( 'Importe les titres, extraits et contenus Wolof des 14 régions (pack mobile). Cochez « Forcer » pour écraser les meta déjà renseignées.', 'vjr221-wolof' ); ?></p>
		<form method="post">
		...
		<hr />
		<form method="post">
			<?php wp_nonce_field( 'vjr221_wo_repair' ); ?>
			<p class="description"><?php esc_html_e( 'Si content_wo affiche « Jëmmalnn… » sans vrais sauts de ligne, utilisez ce bouton (sans réimporter).', 'vjr221-wolof' ); ?></p>
			<p>
				<button type="submit" name="vjr221_wo_seed_repair" class="button" value="1">
					<?php esc_html_e( 'Réparer les sauts de ligne content_wo', 'vjr221-wolof' ); ?>
				</button>
			</p>
		</form>
		<hr />
		<p><strong><?php esc_html_e( 'Test API :', 'vjr221-wolof' ); ?></strong></p>
		<pre style="background:#f6f7f7;padding:12px;overflow:auto">curl -s '<?php echo esc_url( home_url( '/wp-json/vjr221/v1/regions/2153' ) ); ?>'</pre>
	</div>
	<?php
}
...
function vjr221_wo_repair_content_newlines() {
	global $wpdb;
	...
	foreach ( $rows as $row ) {
		$raw   = $row->meta_value;
		$fixed = vjr221_wo_normalize_newlines( $raw );
		if ( $fixed !== $raw ) {
			update_post_meta( (int) $row->post_id, 'content_wo', $fixed );
			$count++;
		}
	}
	return $count;
